...
class LoginViewModel: BaseViewModel, NetworkRequestHandler {
    ...
    func login(username: String, password: String) -> Observable<Result<String?>> {
        return networking.login(username: username, password: password).map{ (response, data) -> Result<(String?)> in
            let userData = UserLoginData(JSON: data as! Dictionary<String, AnyObject>)
